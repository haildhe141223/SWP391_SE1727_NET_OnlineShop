...
namespace SWP391.OnlineShop.Core.Cores.Infrastructures;
        ...
        where TKey : struct
{
    ...
    protected GenericRepository(OnlineShopContext context)
    {
        Context = context;
    ...
    {
        return isHardDeleted == false ? _dbSet.Count(x => x.Status
        != Models.Enums.Status.Inactive) : _dbSet.Count();
    }
    ...
    public virtual async Task<int> CountAsync(bool isHardDeleted = false)
    {
        if (isHardDeleted == false)
            return await _dbSet.CountAsync(x => x.Status
            != Models.Enums.Status.Inactive);
    ...
    public virtual void Delete(int entityId, bool isHardDeleted = false)
    {
        ...
        if (isHardDeleted == false)
        {
            entity.Status = Models.Enums.Status.Inactive;
            Context.Entry(entity).State = EntityState.Modified;
            return;
        }
    ...
    {
        var entityExist = _dbSet.FirstOrDefault(x => entity != null &&
                                                     x.Id.Equals(entity.Id));

        if (entityExist == null && entity != null)
    ...
    public virtual void Delete(bool isHardDeleted = false, params object[] keyValues)
    {
        ...
        if (isHardDeleted == false)
        {
            ...
            entitiesExist.Status = Models.Enums.Status.Inactive;
    ...
    {
        var entitiesExist = await _dbSet.FirstOrDefaultAsync(x => entity != null && x.Id.Equals(entity.Id));

        if (entitiesExist == null && entity != null)
            throw new ArgumentNullException($"{entity.Id} was not found in the {typeof(TEntity)}");

        if (isHardDeleted == false)
        {
            if (entitiesExist == null) return;
            entitiesExist.Status = Models.Enums.Status.Inactive;
            Context.Entry(entitiesExist).State = EntityState.Modified;
        ...
        if (entitiesExist != null) _dbSet.Remove(entitiesExist);
    }
    ...
    {
        var query = _dbSet.Where(condition).ToList();
